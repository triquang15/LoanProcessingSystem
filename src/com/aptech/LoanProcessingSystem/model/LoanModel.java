...
public class LoanModel {
		...
		try {
			loans = new ArrayList<Loan>();
				PreparedStatement preparedStatement = ConnectDB.connection().prepareStatement(
						"SELECT c.Name as CustomerName, lt.name as LoanType, pt.name as PaymentType, l.* FROM `loan` as l join `customer` as c on l.CustomerId = c.Id join `loantype` as lt on l.LoanTypeId = lt.Id JOIN `paymenttype` as pt on l.PaymentTypeId = pt.id");
	...
	public List<Loan> findAllActive() {
		...
		try {
			loans = new ArrayList<Loan>();
				PreparedStatement preparedStatement = ConnectDB.connection().prepareStatement(
						"SELECT c.Name as CustomerName, lt.name as LoanType, pt.name as PaymentType, l.* FROM `loan` as l join `customer` as c on l.CustomerId = c.Id join `loantype` as lt on l.LoanTypeId = lt.Id JOIN `paymenttype` as pt on l.PaymentTypeId = pt.id where l.status < 3");
				ResultSet rs = preparedStatement.executeQuery();
				while (rs.next()) {
					Loan loan = new Loan();
					loan.setId(rs.getInt("Id"));
	...
	public List<Loan> getAllLoansByCustomerID(int id) {
		...
		try {
			...
			PreparedStatement statement = ConnectDB.connection()
					.prepareStatement("" + "Select * from loan where CustomerId = ?");
	...
	public List<Loan> searchByCustomerName(String keyword) {
		...
		try {
			PreparedStatement preparedStatement = ConnectDB.connection().prepareStatement(
					"SELECT c.Name as CustomerName, lt.name as LoanType, pt.name as PaymentType, l.* FROM `loan` as l join `customer` as c on l.CustomerId = c.Id join `loantype` as lt on l.LoanTypeId = lt.Id JOIN `paymenttype` as pt on l.PaymentTypeId = pt.id WHERE c.Name LIKE ? and l.status < 3");
			preparedStatement.setString(1, "%" + keyword + "%");
			ResultSet rs = preparedStatement.executeQuery();
			while (rs.next()) {
				Loan loan = new Loan();
